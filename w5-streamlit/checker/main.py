...
stmt = (
    "INSERT INTO results (nickname, email, score, filename) values (?,"
    " ?, ?, ?)"
)
...
with st.form("my_form"):
    ...
    student_email = st.text_input("Your e-mail")
    uploaded_file = st.file_uploader("Upload csv")
    submitted = st.form_submit_button("Submit")
    info_element = st.info(
        "You must fill the name, mail and"
        "upload the file, then press `Submit`"
    )
    ...
    if (
        ...
        info_element.error(
            "You must fill the name, mail and upload the file, then"
            " press `Submit`"
        )
